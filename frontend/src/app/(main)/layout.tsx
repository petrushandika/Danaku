      ...
      <div className="flex flex-col min-h-screen bg-white dark:bg-slate-950 selection:bg-emerald-100 selection:text-emerald-900 transition-colors duration-300 font-sans">
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              ...
              {isAuthenticated ? (
                ...
                <>
                  <Link
                    href="/auth/login"
                    className="hidden sm:block text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-emerald-600 px-4 transition-colors"
                  >
                    {t.nav.signIn}
                  </Link>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="xl:hidden hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800"
                  >
                ...
                <SheetContent
                  side="right"
                  className="w-80 sm:w-96 dark:bg-slate-950 dark:border-slate-800 [&>button]:hidden"
                >
                      ...
                      {!isAuthenticated && (
                        <div className="flex flex-col gap-3 pt-4">
                          ...
                          >
                            <Button
                              variant="outline"
                              className="w-full rounded-full border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-bold h-12"
                            >
                              {t.nav.signIn}
                            </Button>
                          </Link>
        ...
        <footer className="bg-white dark:bg-slate-950 text-slate-900 dark:text-white py-12 md:py-16 px-6 border-t border-emerald-100 dark:border-slate-800 transition-colors">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
            ...
            <div>
              <h4 className="font-black uppercase tracking-widest text-emerald-600 text-[10px] mb-6 italic">
                {t.footer.solution}
              </h4>
              <ul className="space-y-3">
                {[
                  ...
                  <li key={item}>
                    <Link
                      href="#"
                      className="text-slate-500 dark:text-slate-400 hover:text-emerald-600 transition-colors text-sm font-bold"
                    >
                      {item}
                    </Link>
                  </li>
            ...
            <div>
              ...
              <ul className="space-y-3">
                {["About Us", "Our Story", "Careers", "Press Kit"].map(
                  (item) => (
                    <li key={item}>
                      <Link
                        href="#"
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                <Link
                  key={i}
                  href={social.href}
                  className="w-9 h-9 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center hover:bg-emerald-600 hover:border-emerald-500 transition-all group"
                >
